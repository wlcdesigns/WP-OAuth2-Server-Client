...
add_action('wp_ajax_use-auth', function(){
	
	check_ajax_referer('wp_oauth', 'wpOauthSec');
	//dbug($_POST);
	...
	ob_start();
	echo json_encode(array('status'=>$update_client));
	header('Content-type: application/json');
	ob_end_flush();
...
function wo_create_client( $user=null ){
	...
	$slug = empty($user['client-name']) ? 'No Name '.rand() : $user['client-name'];
