 ...
 * Author URI: https://wlcdesigns.com
 * License: GPL2
...
function _wpoauth_custom_wp_admin_style($hook) {
	
    if($hook != 'toplevel_page_wo_settings') {
            return;
    }
    
    wp_enqueue_style( 'sweetalert-style', plugins_url( '/assets/css/sweetalert2.min.css', __FILE__ ) );
    
    wp_enqueue_script( 'sweetalert',plugins_url( '/assets/js/sweetalert2.min.js', __FILE__ ), array(), '6.4.2', true );
        
}

add_action( 'admin_enqueue_scripts', '_wpoauth_custom_wp_admin_style' );
...
include_once( dirname( __FILE__ ) . '/library/authentication/StandardAuthentication.php' );
