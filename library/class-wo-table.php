			...
			foreach ( $columns as $column_name => $column_display_name ) {
				...
				switch ( $column_name ) {
					case "name": 
						...
						echo '
						<td '.$attributes.'>
							<strong><a class="thickbox" href="' . $edit_link . '" title="Edit">'.stripslashes($rec->name).'</a></strong>
							<div class="row-actions">
								<span class="edit"><a class="thickbox" href="' . $edit_link . '" title="Edit Client">Edit</a> | </span><span class="trash"><a class="submitdelete" title="Delete this Client permanently" onclick="wo_remove_client(\''.str_replace(" ","",$rec->client_id).'\');" href="#">Delete</a> | </span><span class="view"><a href="#TB_inline?width=300&height=100&inlineId=show_secret_'.$rec->client_id.'" class="thickbox" title="Viewing Secret for '.$rec->name.'">Show Secret</a></span> | <span class="slug"><a href="#TB_inline?width=300&height=100&inlineId=show_slug_'.$rec->slug.'" class="thickbox" title="Viewing Slug for '.$rec->name.'">Show Client Slug</a></span></div> <div id="show_secret_'.$rec->client_id.'" style="display:none;"><h3 style="text-align:center;margin-top:40px;">'.$rec->client_secret.'</h3></div><div id="show_slug_'.$rec->slug.'" style="display:none;"><h3 style="text-align:center;margin-top:40px;">'.$rec->slug.'</h3></div>'; break;
				 ...
				 // case "redirect_uri": echo '<td '.$attributes.'>'.$rec->redirect_uri.'</td>'; break;
					...
					case "slug":
						...
						<td '.$attributes.'>
							<label for="use_auth">
								Use built in authentication? <input data-client_id="',$rec->client_id,'" id="use_auth" type="checkbox" ',checked($rec->use_auth,true), '/>
							</label>
							<br />
							<small>You will be required to use your own authentication method if this is unchecked</small>
						</td>';
					break;
				}
			}
		...
		<script type="text/javascript">
			jQuery(function($){
				...
				$("#use_auth").change(function(){
					...
					var data = {
						action: "use-auth",
						wpOauthSec: '<?php echo wp_create_nonce("wp_oauth"); ?>',
						use_auth: checked,
						client_id: client_id
					};
